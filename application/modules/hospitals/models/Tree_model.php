...
class Tree_model extends CI_Model
{
	...
	function generate_node_tree($options = array(), $offset = 0, $child = FALSE)
	{
		...
		foreach ($query as $record)
		{
			...
			$class = ($record['code'] === $options['active']) ? 'active' : '';
			...
			if (isset($options['link'])) {
				...
				if ($this->input->get('type')) {
					$url .= preg_replace('/(^|&)type=[^&]*/', '&type='.$record['code'], $_SERVER['QUERY_STRING']);
				} else {
					$url .= $_SERVER['QUERY_STRING'].'&type='.$record['code'];
				}
	...
	public function get_lineage($id = null, $options = array())
	{
		...
		if ( ! empty($query))
		{
			...
			if ($options['get_data'] === '') {
				...
				$column = $options['get_data'];
				array_push($this->lineage, $query[0]->$column);
			}
		...
		return array_values(array_reverse($this->lineage));
	}
