<?php $this->load->view('admin/header', array(
	...
	'link' => 'hospitals',
	'sub_link' => 'types',
	'pageTitle' => lang('menu_types_facilities'),
	'breadcrumb' => array(
        ['name' => lang('menu_hospitals'), 'link'=> 'admin/hospitals'],
        ['name' => lang('menu_types_facilities'), 'link'=> false],
    ),
	'styles' => array(
...
<div class="text-right">
	<button type="button" class="btn btn-sm btn-primary" data-toggle="modal" data-target="#modal-addType">
		<i class="fa fa-plus mr-1"></i> <?php echo lang('menu_types') ?>
	</button>
</div>
	...
	<div class="table-responsive mt-3">
		<table class="table" id="table">
			<thead>
				<tr>
					<th>Id</th>
					<th><?php echo lang('form_hospital_type') ?></th>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content border-0">
            <div class="modal-header">
                <h5 class="modal-title text-uppercase">Add Company Type</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <?php if (validation_errors() && $this->input->post('add_types')): ?>
                    <div class="alert alert-danger" data-trigger-modal="#modal-add">Correct the errors in the form and try again</div>
                <?php endif ?>
                <?php echo form_open(current_url()) ?>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="form-group <?php echo form_error('insert['.$key.'][description]') ? 'has-error' : null ?>">
                                    <textarea name="insert[<?php echo $key ?>][description]" class="form-control form-control-sm"><?php echo set_value('insert['.$key.'][description]') ?></textarea>
                    ...
                    <input type="submit" name="add_types" value="Create" class="btn btn-primary">
                <?php echo form_close() ?>
            </div>
        </div>
    </div>
...
document.addEventListener("DOMContentLoaded", function() {
	...
	const table = $('#table').DataTable({
		...
		"ajax": "<?php echo site_url('hospitals/api/types') ?>",
		dom: 'Bfrtip',
		buttons: [
			{
				extend: "colvis",
			...
			},
			{
				...
				action: function ( e, dt, button, config ) {
					...
					$.ajax({
						url: '<?php echo base_url('hospitals/api/deleteTypes') ?>',
						type: 'POST',
						data: {
