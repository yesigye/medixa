<?php $this->load->view('admin/header', array(
	...
	'pageTitle' => lang('menu_locations_zones'),
	'link' => 'zones',
	'breadcrumb' => array(
		1 => array('name'=> lang('menu_locations_th'), 'link'=> false),
		2 => array('name'=> lang('menu_locations_zones'), 'link'=> false),
	),
	'styles' => array(
...
<div class="text-right">
	<button type="button" class="btn btn-sm btn-primary" data-toggle="modal" data-target="#modal-addType">
		<i class="fa fa-plus mr-2"></i> <?php echo lang('menu_locations_zones') ?>
	</button>
</div>
...
<?php echo form_open(current_url()) ?>
	<div class="table-responsive mt-3">
		<table class="table" id="table">
			<thead>
				<tr>
					<th>Id</th>
					<th><?php echo lang('form_location_zone') ?></th>
	...
	<button type="submit" name="update_zones" value="submit" class="btn btn-success">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content border-0">
			<div class="modal-header">
				<h5 class="modal-title"><?php echo lang('menu_locations_zones') ?></h5>
			...
			<div class="modal-body table-responsive">
				<?php echo form_open(current_url()) ?>
							...
							<tr>
								...
								<td class="text-center">
									<div class="custom-control custom-checkbox mt-2">
										<?= form_checkbox("insert[$key][status]", 'active', null, ['class'=>"custom-control-input", 'id'=>"check_insert[$key][status]"]) ?>
										<?= form_label('', "check_insert[$key][status]", ['class'=>'custom-control-label']) ?>
										<?php if (form_error("insert[$key][status]")): ?>
											<div class="invalid-feedback"><?php echo form_error("insert[$key][status]") ?></div>
										<?php endif ?>
									</div>
								</td>
								<td class="text-center">
					...
					<input type="submit" name="add_zones" value="<?php echo lang('btn_create') ?>" class="btn btn-primary">
				<?php echo form_close() ?>
			</div>
		</div>
	</div>
...
document.addEventListener("DOMContentLoaded", function() {
	...
	const table = $('#table').DataTable({
		...
		"ajax": "<?php echo site_url('locations/api/zones') ?>",
		dom: 'Bfrtip',
		buttons: [
			{
				extend: "colvis",
