...
<div class="row">
	<div class="col-sm-9 col-md-10">
		<div class="lead mb-3"><?php echo lang('menu_categories') ?></div>
	</div>
	<div class="col-sm-3 col-md-2">
		<button type="button" class="btn btn-sm btn-block btn-primary" data-toggle="modal" data-target="#group-create-modal">
		...
		<div class="modal-dialog">
			<div class="modal-content border-0">
				<div class="modal-header">
					<b><?php echo lang('menu_categories') ?></b>
				...
				<div class="modal-body">
					...
					<button type="submit" name="create" value="1" class="btn btn-block btn-secondary mt-2">
						<?php echo lang('btn_create') ?>
					</button>
				</div>
			</div>
...
document.addEventListener("DOMContentLoaded", function() {
	...
	const table = $('#table').DataTable({
		...
		"ajax": "<?php echo site_url('products/api/categories') ?>",
		dom: 'Bfrtip',
		buttons: [
			{
				extend: "colvis",
			...
			},
			{
				...
				action: function ( e, dt, button, config ) {
					...
					$.ajax({
						url: '<?php echo base_url('products/api/categories/delete') ?>',
						type: 'POST',
						data: {
	...
	$("#group-create").datatableInsert({
		...
		alertId: 'app-notif',
		modal: create_modal,
		urlCreate: '<?php echo site_url('products/api/categories/create') ?>',
	...
	$('#table').on('click', '.delete-row', function() {
		...
		// AJAX to delete item
		$.ajax({
			url: '<?php echo base_url('products/api/categories/delete/') ?>'+row.attr('id'),
			type: 'DELETE',
			success: function(result) {
